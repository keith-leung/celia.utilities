...
namespace Celia.io.Core.MicroServices.Utilities.Webs.Clients
{
    ...
    public abstract class HttpRequestBase<TRequest> where TRequest : IRequest<TRequest> {
        ...
        /// 放入URL参数集合
        /// </summary>
        private IDictionary<string, object> _urlParams;
        ...
        protected HttpRequestBase( HttpMethod httpMethod, string url ) {
            ...
            _urlParams = new Dictionary<string, object>();
        ...
        {
            //调试信息
            System.Diagnostics.Debug.WriteLine($"{_httpMethod.Method}|{_url}");
        ...
        public String Parameter()
        {
            if(this._httpMethod == HttpMethod.Get){
                return Json.ToJson(_urlParams);
            }
            else
            {
                if(_contentType == HttpContentType.FormUrlEncoded.Description()){
        ...
        protected virtual HttpRequestMessage CreateRequestMessage() {
            var message = new HttpRequestMessage {
                Method = _httpMethod,
                RequestUri = CreateRequestUri(_httpMethod,_url ),
                Content = CreateHttpContent(_httpMethod)
            };
        ...
        /// <param name="url">地址</param>
        /// <returns></returns>
        private Uri CreateRequestUri(HttpMethod httpMethod, String url)
        {
            var urlParams = String.Empty;
            foreach (var keyValue in _urlParams)
            {
                urlParams += $"&{keyValue.Key}={keyValue.Value}";
            }
            if (!String.IsNullOrEmpty(urlParams))
            {
                _url = $"{url}?{urlParams.Substring(1)}";
            }
            return new Uri(_url);
        }
        ...
        /// 创建请求内容
        /// </summary>
        private HttpContent CreateHttpContent(HttpMethod httpMethod) {
            ...
            throw new NotImplementedException( "ContentType not implemented" );
        }
